...
import os
from ..utils.constants import BAND_SPATIAL_RESOLUTION_DICT, S2_DEFAULT_QUANTIFICATION_VALUE
...
class L1C_tile():
    ...
    def __init__(self, tile_bands=None, bands_names=None, tile_name=None, tile_coordinates=None, tile_footprint_coordinates=None, bands_file_name_dict=None, crs=None, device=torch.device("cpu")):
        """Initialize an L0 tile.

        Args:
            tile_bands (list, optional): list of torch tensors representing the different bands in the format [H,W]. Defaults to None.
    ...
    def export_to_tif(self, save_path):
        ...
        for band_name in list(self.__bands_dict.keys()):
            ...
            export_band_to_tif(band,  self.__crs, coords, os.path.join(save_path, band_name+".tif"))

    def create_tile(self, bands_list, tile_bands, tile_name, tile_coordinates, footprint_coordinates, bands_file_name_dict, crs):
    ...
    def get_tile_info(self):
        ...
        return tile_name, sensing_time, acquisition_time, self.get_tile_coordinates(), self.get_tile_footprint_coordinates()
    ...
    def as_tensor(self, requested_bands=None, downsampling=True):
        ...
        if downsampling:
            ...
            bands_resolution_list=[BAND_SPATIAL_RESOLUTION_DICT[b] for b in requested_bands]
            lowest_resolution=max(bands_resolution_list)

            band_shape=[self.get_band(b) for b in requested_bands][bands_resolution_list.index(lowest_resolution)].shape
            bands=torch.zeros([band_shape[0], band_shape[1], len(requested_bands)], device=self.__device)
            ...
            resolution_min=min([BAND_SPATIAL_RESOLUTION_DICT[b] for b in requested_bands])

            band_shape=[max(self.get_band(b).shape[0] for b in requested_bands), max(self.get_band(b).shape[1] for b in requested_bands)]
            ...
            for band_name, n in zip(requested_bands, range(len(requested_bands))):
                ...
                if resolution_min != BAND_SPATIAL_RESOLUTION_DICT[band_name]:
                    band_upsampled=image_band_upsample(band, band_name, resolution_min, "bilinear")
    ...
    def show_bands_superimposition(self, requested_bands=None, downsampling=True):
        ...
        if bands_superimposed.shape[2] == 2:
            bands_superimposed_zeros=torch.zeros([bands_superimposed.shape[0], bands_superimposed.shape[1],3])
            bands_superimposed_zeros[:,:,:2]=bands_superimposed
            bands_superimposed_zeros[:,:,2]=(bands_superimposed[:,:,0] + bands_superimposed[:,:,1])/2
