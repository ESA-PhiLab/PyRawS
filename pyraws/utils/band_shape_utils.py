...
def image_band_upsample(img_band, band_name, target_spatial_resolution, upsample_mode): 
    ...
    upsample_factor=BAND_SPATIAL_RESOLUTION_DICT[band_name]/target_spatial_resolution
    if not(upsample_mode in ['nearest', 'bilinear', 'bicubic']):
        raise ValueError("Upsample mode "+colored(upsample_mode, "blue")+" not supported. Please, choose among: ""nearest"", ""bilinear"", ""bicubic"".")
        
    
    if upsample_factor  <= 1.0:
        print(colored("Warnings", "red")+". The requested target resolution ("+colored(target_spatial_resolution, "blue") + ") is lower or equal to the orginal band resolution ("+colored(band_name, "red")+","+colored(BAND_SPATIAL_RESOLUTION_DICT[band_name], "green")+").")
        return img_band
    
    if upsample_factor != int(upsample_factor):
        print(colored("Warnings", "red")+". Upsample factor truncanted from "+upsample_factor+" to "+int(upsample_factor)+".")
    #else:
...
def image_band_resize(img_upsample_band, band_name, upsampled_img_spatial_resolution, interpolate_mode): 
    ...
    downsample_factor=BAND_SPATIAL_RESOLUTION_DICT[band_name]/upsampled_img_spatial_resolution
    
    if not(interpolate_mode in ['nearest', 'bilinear', 'bicubic']):
        raise ValueError("Interpolate mode "+colored(interpolate_mode, "blue")+" not supported. Please, choose among: ""nearest"", ""bilinear"", ""bicubic"".")
        
    
    if downsample_factor  <= 1.0:
        print(colored("Warnings", "red")+". The upsampled image resolution ("+colored(upsampled_img_spatial_resolution, "blue") + ") is lower or equal to the orginal band resolution ("+colored(band_name, "red")+","+colored(BAND_SPATIAL_RESOLUTION_DICT[band_name], "green")+").")
        return img_upsample_band
    
    if downsample_factor != int(downsample_factor):
        print(colored("Warnings", "red")+". Upsample factor truncanted from "+downsample_factor+" to "+int(downsample_factor)+".")
    #else:
        #print("Downsample factor: "+colored(downsample_factor, "blue")+".")
    downsample_factor=int(downsample_factor)
    
    size=(int(img_upsample_band.shape[0]/downsample_factor), int(img_upsample_band.shape[1]/downsample_factor))
    with torch.no_grad():
        return interpolate(img_upsample_band.unsqueeze(0).unsqueeze(0),size=size, mode=interpolate_mode, align_corners=True).squeeze(0).squeeze(0)
