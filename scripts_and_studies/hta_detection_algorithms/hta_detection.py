# [1] - Liu, Yongxue, et al. "Detecting high-temperature anomalies from Sentinel-2 MSI images."
# ISPRS Journal of Photogrammetry and Remote Sensing 177 (2021): 174-193.
...
def get_TAI_mean(tai):
    """It returns the TAI mean as defined in [1]. TAI_mean is calculated by using a 15x15 convolutional filter.
    The input image is padded with the values at the boundaries to avoid boundaries false positives.
    ...
    weight = torch.nn.Parameter(
        1 / 31 * torch.ones([31, 31]).unsqueeze(0).unsqueeze(0), requires_grad=False
    )
...
def get_TAI_buffer(tai, tai_mean):
    ...
    with torch.no_grad():
        ...
        padding = torchvision.transforms.Pad(
            (15, 15, 15, 15), padding_mode="constant", fill=0
        )
        s_pad = padding(tai)
...
def cluster_9px(img):
    ...
    with torch.no_grad():
        ...
        surrounded = torch.where(
            surrounded >= 0.45, 0.45 * torch.ones_like(surrounded), surrounded
        )
        del weight
        del conv
        del img_pad
...
def manage_saturated_thermal_anomalies(s2_l1c_img):
    ...
    """
    saturated_mask = torch.logical_and(
        torch.where(s2_l1c_img[:, :, 2] >= 1, 1, 0),
        torch.where(s2_l1c_img[:, :, 2] >= 1, 1, 0),
    )
    saturated_mask_8_connected = cluster_9px(saturated_mask)
    return torch.logical_or(
        saturated_mask_8_connected, torch.logical_not(saturated_mask)
    )
...
def get_image_lost_source_mask(s2_l1c_img):
    ...
    """
    source_lost_mask_negated = torch.logical_or(
        torch.where(s2_l1c_img[:, :, 0] < 0.01, 1, 0),
        torch.where(s2_l1c_img[:, :, 1] < 0.05, 1, 0),
    )
...
def remove_false_positives(s2_l1c_img, thermal_anomalies_map):
    ...
    thermal_saturated_mask_no_saturation = torch.logical_and(
        thermal_anomalies_map, thermal_saturated_mask
    )
    source_lost_mask = get_image_lost_source_mask(s2_l1c_img)
    thermal_anomalies_no_lost_no_saturation = torch.logical_and(
        thermal_saturated_mask_no_saturation, source_lost_mask
    )
    # Removing pixel affected by NaN
    nan_mask = torch.logical_not(
        torch.logical_or(
            torch.logical_or(
                torch.isnan(s2_l1c_img[:, :, 0]), torch.isnan(s2_l1c_img[:, :, 1])
            ),
            torch.isnan(s2_l1c_img[:, :, 2]),
        )
    )
    thermal_anomalies_clean_map = torch.logical_and(
        thermal_anomalies_no_lost_no_saturation, nan_mask
    )
...
def extract_high_tempearature_anomalies(s2_l1c_img):
    ...
    thr_1 = torch.logical_and(
        torch.where(
            tai >= 0.45 * torch.ones_like(tai),
            torch.ones_like(tai),
            torch.zeros_like(tai),
        ),
        b,
    )  # Selecting pixels in the buffer meeting TAI >= 0.45.
    thr_2 = torch.where(
        s2_l1c_img[:, :, 2] > 2 * s2_l1c_img[:, :, 1] - s2_l1c_img[:, :, 0],
        torch.ones_like(tai),
        torch.zeros_like(tai),
    )  # Condition p_far_SWIR - p_near_SWIR > p_near_SWIR - p_NIR.
    thr_3 = torch.where(
        s2_l1c_img[:, :, 2] > 0.15 * torch.ones_like(tai),
        torch.ones_like(tai),
        torch.zeros_like(tai),
    )  # Removing low reflectivity surfaces in FAR SWIR.
