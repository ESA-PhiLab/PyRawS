...
def parse_coordinate(coordinate_str):
    lon=float(coordinate_str[coordinate_str.find('[')+1:coordinate_str.find(',')-1])
    lat=float(coordinate_str[coordinate_str.find(',')+1:coordinate_str.find(']')-1])
    return (lon, lat)
...
def main():
    ...
    parser.add_argument('--eruption_cvs', type=str, help='Eruption list CSV.', default="eruption_selected.csv")
    parser.add_argument('--json_dir_path', type=str, help='Path to the directory where JSON files are stored.', default="./json")
    ...
    for eruption in csv_content:
        ...
        with open(os.path.join(json_dir, eruption["Name"].replace(" ", "_").replace("/", "_")+".json"), 'w') as f:
