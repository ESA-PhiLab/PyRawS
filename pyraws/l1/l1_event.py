...
from ..utils.l1_utils import get_l1C_image_default_path, read_L1C_event_from_database, read_L1C_event_from_path, reproject_raster
...
class L1C_event():
    ...
    def __init__(self, tiles_collection=None, bands_names=None, event_class=None, event_name=None, device=torch.device("cpu")):
        ...
        if tiles_collection is None:
            ...
            for n in range(len(tiles_collection)):
                self.__tiles_collection.append(torch.device(tiles_collection[n], device=device))
    ...
    def from_path(self, l1c_dir_path, bands_list, reproject_bounds=True, verbose=True):
        ...
        try:
            tiles_collection_dict, tiles_coordinates_dict, footprint_coordinates_dict,bands_file_name_dict_dict=read_L1C_event_from_path(l1c_dir_path, bands_list, reproject_bounds, verbose, device=self.__device)
        except:
            raise ValueError("Impossible to open the L1C file at: "+colored(l1c_dir_path, "red")+".")
        ...
        for tile_name in tiles_names:
            ...
            new_tile.create_tile(bands_list, tile, tile_name, tiles_coordinates_dict[tile_name], footprint_coordinates_dict[tile_name], bands_file_name_dict_dict[tile_name], crs)
            self.__tiles_collection.append(new_tile)
            
        self.n_tiles=len(self.__tiles_collection)
            
    def from_database(self, id_event, bands_list=None, cfg_file_dict=None, id_l0_l1_dict=None, reproject_bounds=True, verbose=True, database="THRAWS"):
        ...
        tiles_collection_dict, tiles_coordinates_dict, footprint_coordinates_dict,bands_file_name_dict_dict,event_class=read_L1C_event_from_database(id_event, bands_list, cfg_file_dict, id_l0_l1_dict, database, reproject_bounds, verbose, device=self.__device)
        tiles_names=list(tiles_collection_dict.keys())
        self.__bands_names=bands_list
        self.__event_class=event_class
    ...
    def show_tiles_info(self):
        ...
        for n in range(len(tiles_names)):
            ...
            for n in range(len(coordinates)):
                print(colored("\tP_"+str(n), "blue") + " : " + colored(str(coordinates[n])+"\n", "red"))
            print("\n")
            print("Footprint's coordinates: \n")
            for n in range(len(footprint_coordinates)):
                print(colored("\tP_"+str(n), "blue") + " : " + colored(str(footprint_coordinates[n])+"\n", "red"))
            print("\n")
    ...
    def merge_tiles_band(self, band_name, merge_path):
        ...
        for n in range(N_granules):
            ...
            tmp_tif_filename=tile_n.get_tile_info()[0]+"_"+band_name+"_"+timestamp+"_tmp.tif"
            src_files_to_mosaic.append(tmp_tif_filename)
            band_n_raster=rasterio.open(tile_n.get_bands_file_name_dict()[band_name], "r")
        ...
        out_meta.update({"driver": "GTiff", "height": mosaic.shape[1], "width": mosaic.shape[2], 
                     "transform": out_trans, "crs": raster_dataset[0].crs})
    ...
    def crop_tile(self, l0_granule_coordinates, out_dir_path=None, out_name_ending=None, lat_lon_format=True, verbose=True, cfg_file_dict=None, id_l0_l1_dict=None, database="THRAWS", overwrite=False):
        """Create a mosaic of l1 tiles and crop the mosaic by using the l0_granule_coordinates. Tge output file is saved as TIF file into the directory specified through ""out_dir_path"".
        ...
        if out_dir_path is None:
            out_dir_path = get_l1C_image_default_path(self.__event_name, cfg_file_dict, id_l0_l1_dict, database)
        else:
        ...
        if not(overwrite) and os.path.exists(out_name):
            print(colored("Warning:", "red"),"the file "+colored(out_name, "blue")+" is already existing. The L1-C crop generation is skipped.")
            return out_name
        ...
        for band_name in band_range:
            if verbose:
                print("Creating a mosaic of different tiles for band: "+colored(band_name, "red"))
        ...
        bands_resolution_list=[BAND_SPATIAL_RESOLUTION_DICT[b] for b in bands_to_merge]
        ...
        with rasterio.open(out_name, "w", **band_meta_list[band_resolution_index]) as dest:
            for band,n in zip(band_cropped_list_resampled, range(len(band_cropped_list_resampled))):
