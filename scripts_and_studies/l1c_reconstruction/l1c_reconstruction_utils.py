...
def get_intersection_points(l1c_tif):
    ...
    return [torch.tensor([dy, k], device=l1c_tif.device), torch.tensor([m,dx], device=l1c_tif.device), torch.tensor([dy_end, l], device=l1c_tif.device),torch.tensor([q, dx_end], device=l1c_tif.device)]
...
def rotate_l1c_tif(l1c_tif):
    ...
    rotated = cv2.warpAffine(l1c_tif.detach().numpy(), M, (int(d[1]), int(d[0])),
...
def get_event_granules_dict(l1c_tif_files):
    ...
    """
    events_granule_list=[x.split(os.path.sep)[-1].split(".tif")[0] for x in l1c_tif_files]
