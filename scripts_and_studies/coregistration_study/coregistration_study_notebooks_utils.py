...
def extract_test_time(test_name):
    ...
    result_dict={"n_images" : n_images, "coreg_type" : coreg_type, "device" : device, "equalize" : equalize, "result [s]" : results}
...
def generate_histograms(results_df, satellite, detector, path="histograms", bins=25):
    ...
    results_satellite_detector = results_df_satellite[results_df_satellite['detector_number']== detector]
    ...
    plt.savefig(os.path.join(path,"hist_" + str(satellite) + "_" + str(detector) +"_Nv.png"))
    plt.figure()
    plt.hist(results_satellite_detector_N_h, bins=bins)
    plt.xlabel("N_h")
    plt.ylabel("Number of occurrences")
    plt.title("Hist_"+ str(satellite) + "_" + str(detector) +"_Nh")
    plt.savefig(os.path.join(path,"hist_" + str(satellite) + "_" + str(detector) +"_Nh.png"))
    return 