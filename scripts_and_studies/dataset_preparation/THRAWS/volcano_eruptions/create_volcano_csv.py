...
def parse_eruption(eruption):
    ...
    if (sentinel2_starting_date >= start_date) and (sentinel2_starting_date >= end_date): # I check both to be sure there are not errors in the excel.
        discard=True
    
    coordinates={"lat" : float(eruption["Latitude"]), "lon" : float(eruption["Longitude"])}

    return [volcano_name, start_date, end_date, coordinates, discard]

def create_rectangular_polygon(center_coordinates, center_distance_x, center_distance_y):

    center_distance = np.sqrt(center_distance_x * center_distance_x + center_distance_y * center_distance_y)
    bearing_angle_0 = 90 - np.arcsin(center_distance_y / center_distance) * 180 / np.pi
    print(center_distance)

    bearing_angles={"SW" : 180 + bearing_angle_0, "NW" : 360 - bearing_angle_0, "NE" : bearing_angle_0, "SE" : 180 - bearing_angle_0}
    print(bearing_angles)
    polygon_dict={"SW" : 0, "NW" : 0, "NE" : 0, "SE" : 0} #South-West, North-West, ...
    for bearing_keys in list(bearing_angles.keys()):
        polygon_vertex=geopy.distance.distance(meters=center_distance).destination((center_coordinates["lat"], center_coordinates["lon"]), bearing=bearing_angles[bearing_keys])
...
def get_eruptions_info_list(eruption_list_csv_name, n_max_volcano, target_csv_filename="eruption_selected.csv", eruption_day_length=2, polygon_semidiagonal_km=7.0):
    ...
    for eruption in eruptions_list:
        ...
        if volcanos_count_dict[volcano_name] < n_max_volcano:
            ...
            if eruption_info[-1]:
                print("Skipping volcano eruption: "+colored(volcano_name, "red")+ "for unacceptable dates Start date("+colored(eruption_info[-4], "blue")+"), End date("+colored(eruption_info[-3], "green")+").")
            ...
            eruption_info=parse_eruption(eruption)
            skipped_eruptions_list.append(eruption_info[:-1])
            print("Skipping volcano eruption: "+colored(volcano_name, "red")+ "for passing the number of eruptions for volcano ("+colored(n_max_volcano, "blue")+".")

    fieldnames=["Name", "Starting date", "End date", "Pol-SW", "Pol-NW", "Pol-NE", "Pol-SE"]

    volcanos_id_cnt=dict(zip(list(volcanos_count_dict.keys()), [0 for n in range(len(list(volcanos_count_dict.keys())))]))
    ...
    with open(target_csv_filename, mode='w') as csv_file:
        ...
        for eruption_info in eruptions_info_list:
            ...
            start_date=str(start_date.year)+"-"+str(start_date.month)+"-"+str(start_date.day)+"T00:00:00"
            end_date=str(end_date.year)+"-"+str(end_date.month)+"-"+str(end_date.day)+"T23:59:59"
            polygon=create_polygon(center_coordinates, polygon_semidiagonal_km*1000)
            eruption_record=[volcano_id, start_date, end_date, polygon["SW"], polygon["NW"], polygon["NE"],polygon["SE"]]
...
def main():
    ...
    parser.add_argument('--eruption_cvs', type=str, help='Eruption list CSV.', default="eruption_list.csv")
    parser.add_argument('--n_max_volcano', type=int, help='Maximum number of eruption per volcano.', default=5)
    parser.add_argument('--eruption_day_length', type=int, help='Number of days margin per eruption.', default=2)
    parser.add_argument('--pol_distance_km', type=float, help='Distance of a vertex from the polygon center in km (semidiagonal).', default=7.0)
    ...
    get_eruptions_info_list(eruption_list_csv_name, n_max_volcano, target_csv_filename="eruption_selected.csv", eruption_day_length=eruption_day_length, polygon_semidiagonal_km=polygon_semidiagonal_km)
