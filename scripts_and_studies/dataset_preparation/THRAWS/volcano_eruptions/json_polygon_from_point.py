...
from .create_volcano_csv import create_rectangular_polygon


def create_polygon_from_coordinates(
    lat, lon, center_distance_x, center_distance_y, json_name=None
):
...
def main():
    ...
    parser.add_argument(
        "--dx_km",
        type=float,
        help="Horizontal distance from the polygon center in km.",
        default=5.0,
    )
    parser.add_argument(
        "--dy_km",
        type=float,
        help="Vertical distance from the polygon center in km.",
        default=14.0,
    )
    parser.add_argument(
        "--json_name", type=str, help="json file name.", default="my_json_polygon.json"
    )
    pargs = parser.parse_args()
    create_polygon_from_coordinates(
        pargs.lat, pargs.lon, pargs.dx_km * 1000, pargs.dy_km * 1000, pargs.json_name
    )
